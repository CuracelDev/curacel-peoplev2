...
const typeConfig: Record<AssessmentType, { label: string; icon: React.ComponentType<{ className?: string }>; color: string }> = {
  ...
  WORK_TRIAL: { label: 'Work Trial', icon: Briefcase, color: 'bg-success/10 text-success' },
    ...
    <div className="space-y-6">
      ...
      <Card>
        <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-4">
          ...
          <Link href="/hiring/settings/assessments/new">
            <Button>
                ...
                {filteredTemplates.map((template) => {
                  ...
                  return (
                    <TableRow key={template.id}>
                      ...
                      <TableCell>
                        <Badge className={template.isActive ? 'bg-success/10 text-success' : 'bg-muted text-foreground/80'}>
