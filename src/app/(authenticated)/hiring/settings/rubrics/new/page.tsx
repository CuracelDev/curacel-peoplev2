...
export default function NewRubricPage() {
  ...
  const createRubricMutation = trpc.interviewStage.createTemplate.useMutation({
    onSuccess: () => {
      ...
      router.push('/hiring/settings/rubrics')
    },
    onError: (error) => {
    ...
    <div className="max-w-4xl mx-auto space-y-6">
      ...
      <div className="flex items-center gap-4">
        ...
        <div className="flex-1">
          <h1 className="text-2xl font-semibold">Create Interview Rubric</h1>
          <p className="text-muted-foreground">Define scoring criteria for consistent candidate evaluation</p>
        </div>
        <Button variant="outline" asChild>
          <Link href="/hiring/settings/rubrics">Cancel</Link>
        </Button>
        <Button
          onClick={handleSave}
          disabled={!rubricName || !rubricStage || createRubricMutation.isPending}
        >
